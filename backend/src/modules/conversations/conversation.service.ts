...
const findOrCreateConversation = async (participantAId: string, participantBId: string) => {
    ...
    if (!conversation) {
        ...
        throw new AppError(StatusCodes.INTERNAL_SERVER_ERROR, 'Could not create or find conversation');
    }

    // Mark messages as read for the current user (participantAId).
    await Message.updateMany(
        { conversationId: conversation._id, senderId: { $ne: participantAId }, isRead: false },
        { isRead: true },
    );
